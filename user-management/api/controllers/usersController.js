  ...
  User.findOne(query)
    ...
    .then((foundUser) =>
      responseUtil._checkExistedData(
        foundUser,
        response,
        process.env.ERROR_FIND_USER
      )
    )
    .then((existedUser) =>
      responseUtil._checkPasswordUser(existedUser, req.body.password)
    )
...
const enableMFA = function (req, res) {
  ...
  User.findByIdAndUpdate(req.body.id, query)
    ...
    .then((generatedQrCode) =>
      responseUtil._getSuccessResponse(generatedQrCode, response)
    )
    .catch((error) => responseUtil._getErrorResponse(error, response))
    .finally(() => responseUtil._sendReponse(res, response));
};
...
const _generateToken = function (dbUser) {
  ...
  return sign(
    { username: dbUser.username, roles: dbUser.roles },
...
const _generateSecret = function (req) {
  return speakeasy.generateSecret({
    name: process.env.SECRET_KEY_MFA + req.user.username,
  });
};
