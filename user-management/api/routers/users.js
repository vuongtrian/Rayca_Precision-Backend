...
router.route(process.env.USER_ID_ROUTE).get(usersController.getOne);
router.route(process.env.LOGIN_ROUTE).post(usersController.login);
router
  .route(process.env.AUTHORIZE_ROUTE)
  .post(
    ...
    usersController.testAuthorization
  );
router
  .route(process.env.ENABLE_MFA_ROUTE)
  .post(authenticateController.authenticate, usersController.enableMFA);
router
  .route(process.env.VERIFY_MFA_ROUTE)
  .post(
    authenticateController.authenticate,
    mfaAuthorizationController.verifyMFA
  );
